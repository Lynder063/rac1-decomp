...
/* readBufEndGet: consumes up to n bytes from the ring buffer and returns
   how many. It returns int, as in the sample; the return is what keeps
   retail's register copy. */
int func_0023CDF0(char *arg0, int n) {
    int *b = (int *)(arg0 + 0x50000);
    int taken = (n < b[1]) ? n : b[1];
    b[1] -= taken;
    return taken;
}
