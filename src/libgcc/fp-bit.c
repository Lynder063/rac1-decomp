...
nan ()
{
  /* rac1-decomp modification (see src/libgcc/README.md): retail links
     ONE shared nan object -- every module's reference resolves to the
     same bss address -- where this revision has a static per module.
     The code emitted is identical either way (%hi/%lo of an object);
     only the relocation's target changes. */
