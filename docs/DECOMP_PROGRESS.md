...
| `func_00217EC0` | text | **close, not exact** | `*(int*)(D_0013CA40+0x190)=0; *(short*)(D_0013CA40+0x18E)=0;`. Logic correct but retail computes the two field addresses independently (different `lui`/`addiu` pairs per store) where this compiler reuses one materialized base pointer for both — 40% mismatch, reverted rather than kept as misleading "close" C; not one of the existing documented near-miss categories, flagged for a future look rather than guessed at further. |
...
| everything else in `core_text`/`text` | core_text, text | not started | Still `INCLUDE_ASM` stubs. ~1545 functions total remaining. |
